...
class BSQuery {
	...
	findAll(selector) {
		...
		return Array.from(elements).map(element => new BSQuery(element))
	}
	 ...
	 * @returns {BSQuery|string} The current BSQuery instance for chaining when setting HTML content, or the current inner HTML when getting.
	 */
	html(htmlContent) {
