...
export class StatisticsItem extends ChildComponent {
	...
	render() {
		...
		$BS(this.element).addClass(styles[this.variant]).addClass('fade-in')
		$BS(this.element).find('#statistic-label').text(this.label)
		$BS(this.element).find('#statistic-value').text(this.value)
		return this.element
	}
}
