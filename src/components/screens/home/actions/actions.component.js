...
export class Actions extends ChildComponent {
	...
	updateBalance(event, type) {
		...
		$BS(event.target).text('Sending...').attr('disabled', true)
		const inputElement = $BS(this.element).find('input')
		const amount = inputElement.value()

		if (!amount) {
			validationService.showError($BS(this.element).find('label'))
			return
		}
		...
		$BS(event.target).removeAttr('disabled').text(type)
	}
